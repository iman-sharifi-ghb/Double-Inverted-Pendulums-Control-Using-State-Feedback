...
Bbar = [B;zeros(2,2)];
rank(ctrb(Abar,Bbar))
desired_poles = [-1+1j -1-1j -2+2j -2-2j -2 -2 -5 -5];
K = place(Abar,Bbar,desired_poles);
Bb = [zeros(6,2);eye(2,2)];
%% LINEAR ODE45
init = [0 0 5*3.14/180 -5*3.14/180 10*3.14/180 -10*3.14/180 0 0];
...
Yr2 = 0.3*sign(sin(0.2*t));
figure
plot(t,Yr1,t,X(:,1)/3.98)
figure
plot(t,Yr2,t,X(:,3)/6.636)
figure
plot(t,Yr1,t,XX(:,1))
figure
plot(t,Yr2,t,XX(:,3))
...
function dx = nonlinear_ode(t,x,K)
    ...
    dx(5) = dq3;
    dx(6) = ddq(3);
    dx=[dx(1);dx(2);dx(3);dx(4);dx(5);dx(6);Yr-[x(1);x(3)]];
end
function dX = linear_ode(t,X,Abar,Bbar,Bb,K)
    Yr = 0.3*sign(sin(0.2*t))*[1;1];
    u = -K*X;
    dX = Abar*X + Bbar*u + Bb*Yr;
end
